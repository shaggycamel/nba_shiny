...
INNER JOIN (SELECT * FROM nba.player_game_log WHERE game_date >= CURRENT_DATE - 15) AS player_stats_two_weeks
    ON cte_fantasy_rosters.nba_id = player_stats_two_weeks.player_id
    
GROUP BY cte_fantasy_rosters.competitor_id, cte_fantasy_rosters.competitor_name, cte_fantasy_rosters.player_fantasy_id, player_stats_two_weeks.player_id, cte_fantasy_rosters.player_name
