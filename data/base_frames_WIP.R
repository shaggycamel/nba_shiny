...
save(list = str_subset(objects(), "df_nba"), file = here("base_data.RData"))
...
cat("\t- df_fty_base\n")
df_fty_base <<- nba.dataRub::dh_getQuery(db_con, "SELECT * FROM fty.league WHERE season = '{cur_season}'") 
saveRDS(df_fty_base, here::here("fty_base.RDS"))


#### Map leagues
walk2(df_fty_base$platform, df_fty_base$league_id, \(platform, league_id){
  print(paste0(platform, "; ", league_id))
  
  # Fantasy competitor -------------------------------------------------
  cat("\t- df_fantasy_competitor\n")
  df_fty_competitor <- nba.dataRub::dh_getQuery(db_con, glue::glue(readr::read_file(here("data/sql/fty_league_competitor.sql"))))
  
  # Fantasy league schedule -------------------------------------------------
  cat("\t- df_fantasy_schedule\n")
  df_fty_schedule <- nba.dataRub::dh_getQuery(db_con, glue::glue(readr::read_file(here("data/sql/fty_league_schedule.sql")))) |>
    dplyr::select(-tidyselect::ends_with("_name"))
  
  # Fantasy competitor roster -------------------------------------------------------
  cat("\t- df_fty_roster\n")
  df_fty_roster <- nba.dataRub::dh_getQuery(db_con,  glue::glue(readr::read_file(here("data/sql/fty_team_roster.sql")))) |>
    dplyr::select(-c(competitor_name, opponent_name))
  
  # Fantasy Box Score -------------------------------------------------------
  cat("\t- df_fty_box_score\n")
  df_fty_box_score <- nba.dataRub::dh_getQuery(db_con, glue::glue(readr::read_file(here("data/sql/fty_box_score.sql")))) |>
    dplyr::relocate(tidyselect::starts_with("competitor"), .before = matchup) |>
    dplyr::select(-tidyselect::matches("r_name|r_abbrev"))
  
  # Save data objects
  save(list = str_subset(objects(), "df_fty"), file = here(paste0("fty_", platform, "_", league_id, ".RData")))
