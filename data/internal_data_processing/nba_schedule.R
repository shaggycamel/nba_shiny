return_datasets <- function(base_parameters) {
  ...
  df_matchup_game_count <- df_nba_schedule |>
    ...
    mutate(matchup_games_remaining = game_date >= base_parameters$cur_date) |>
    summarise(
  ...
  lst(df_matchup_game_count, tbl_matchup_games)
}

return_datasets(base_parameters)
